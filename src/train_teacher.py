...
#     if not norm_path.exists():
#         # safe defaults if not present
#         return NormConfig(clip_p_lo=1.0, clip_p_hi=99.0, use_global_zscore=not use_minmax,
#                           global_mean=0.0, global_std=1.0, use_minmax=use_minmax)
...
#         prob = sliding_window_predict_proba(
#             model=model,
#             image=img_norm,              # already normalized
#             mask_breast=breast,
#             preprocess=None,
#             cfg=tile_cfg,
#         )
...
#     df = pd.DataFrame(rows)
#     # summary: mean over images (note: if you cap_mm, distance means reflect that choice)
#     summary = {
#         "n_images": int(len(df)),
#         "mean_dice": float(df["dice"].mean()) if len(df) else float("nan"),
#         "mean_iou": float(df["iou"].mean()) if len(df) else float("nan"),
...
#     # Targets / normalization
#     ap.add_argument("--target_key", type=str, default="mask_mass", choices=["mask_mass", "mask_any"])
#     ap.add_argument("--use_minmax", action="store_true", help="Use per-image minmax after clipping instead of global z-score")
...
#     # Set epoch_len so one epoch has exactly steps_per_epoch batches
...
#         train_ds,
#         batch_size=args.batch_size,
#         shuffle=False,  # dataset already randomizes by idx seed+idx
#         num_workers=args.num_workers,
#         pin_memory=True,
#         drop_last=True,
#     )
...
#     opt = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
#     # simple cosine schedule (stable in practice)
#     sched = torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=max(1, args.epochs))
...
#             # meta is list of dicts (collated)
#             # try to count tags
#             try:
#                 tags = meta["tag"]
#                 for t in tags:
...
#             if global_step % 20 == 0:
#                 lr = float(opt.param_groups[0]["lr"])
#                 pbar.set_postfix(loss=running / max(1, (global_step % (args.steps_per_epoch * 1000))),
#                                  lr=lr)
...
#             # write per-image val metrics each epoch (overwrite)
#             df_val.to_csv(out_logs / "teacher_val_metrics.csv", index=False)
#             (out_logs / "teacher_val_summary.json").write_text(json.dumps(val_summary, indent=2))
